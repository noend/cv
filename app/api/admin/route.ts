...
export async function GET(request: NextRequest) {
  if (!isDev) {
    return NextResponse.json({ error: 'Admin API only available in development mode' }, { status: 403 });
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!file || !data) {
      return NextResponse.json({ error: 'Missing file or data parameter' }, { status: 400 });
    }

    // Validate file path to prevent directory traversal
    const allowedFiles = ['cv-data.ts', 'topSkills.ts', 'profile.ts'];
